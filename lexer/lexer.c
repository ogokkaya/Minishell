...
/*   Updated: 2024/07/04 00:10:36 by onurgokkaya      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// fonksiyon leakleri bakılıcak / doğru çalışdığına bakılıcak /çıkış durumları kontrol edilicek
static void  parse_command_tokens(char *input,t_mshell *shell)
{
    ...
    while(input && *input)
    {
        ...
        if(!content)
            return(printf(MALLOC), end_malloc(shell), exit(1));
        complete_content = ft_strtrim(content, " ");
        free(content);
        ft_lstadd_back_lexer(&shell->lexer, ft_lstnew_lexer(shell,complete_content));
        next_input_tmp = ft_strtrim(input + len, " ");
        if(!next_input_tmp)
            return(printf(MALLOC),free(input), end_malloc(shell), exit(1));
        free(input);
...
void lexer(t_mshell *shell)
{
    ...
    if(!input_tmp)
        return(printf(MALLOC),free(shell->input), end_malloc(shell), exit(1));
